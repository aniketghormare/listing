...
const addOrderItem = async (req, res) => {
  ...
  const address = await OrderItem.create(req.body);
...
const getAllOrderItem = async (req, res) => {
  ...
  let user = await OrderItem.findAll( {where: { uid }});
...
const getOneOrderItem = async (req, res) => {
  ...
  let address = await OrderItem.findOne({ where: { id: id } });
  res.status(200).send(address);
};

const updateOrderItem = async (req, res) => {
  let id = req.params.id;
  let address = await OrderItem.update(req.body, { where: { id: id } });
  res.status(200).send(address);
};
